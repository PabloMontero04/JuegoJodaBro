...
public class main extends JFrame {
    ...
    public main() {
        ...
        enemies.add(new Enemy(1287, 475, 50, 50, "src/img/enemigo.png"));
        ...
        Timer timer = new Timer(10, (e) -> {
            ...
            if ((backgroundX + characterX >= 1000 && backgroundX + characterX<= 1115 || backgroundX+ characterX >= 1565 && backgroundX+ characterX <= 1610) && characterY >= 485) {
         ...
         for (Enemy enemy : enemies) {
        	 enemy.move();
             if (collisionWithEnemy(enemy)) {
                 // Acciones cuando hay una colisión con el enemigo (por ejemplo, decrementar vidas y reiniciar posición)
                 lives--;
                 if (lives <= 0) {
                     // El jugador ha perdido
                     JOptionPane.showMessageDialog(this, "¡Game Over!");
 ...
 // Método para verificar la colisión con un enemigo específico
    private boolean collisionWithEnemy(Enemy enemy) {
        Rectangle characterBounds = new Rectangle(characterX, characterY, characterWidth, characterWidth);
        Rectangle enemyBounds = new Rectangle(enemy.getX() - backgroundX, enemy.getY(),
                enemy.getWidth(), enemy.getHeight());

        return characterBounds.intersects(enemyBounds);
    }
    ...
    public void paint(Graphics g) {
     ...
     // Dibujar los enemigos en el buffer de imagen
        for (Enemy enemy : enemies) {
            g2d.drawImage(enemy.getImage(), enemy.getX() - backgroundX, enemy.getY(),
